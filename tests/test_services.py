...
from backend.services.data_processing import build_daily_demand_series, detect_transaction_outliers, estimate_lost_demand
from backend.services.forecasting import median_baseline, rolling_origin_backtest
from backend.services.replenishment import calculate_inventory_position, calculate_order_quantity, calculate_safety_stock


def test_manual_control_example():
    safety = calculate_safety_stock(0.95, 4, 10, z=1.6449)
    order = calculate_order_quantity(100, round(safety), calculate_inventory_position(40, 30, 5), 12, 48)
...
def test_outlier_detection_is_per_sku_and_repeated_large_sales_remain():
    rows = [{"date": f"2026-01-{i:02d}", "sku": "A", "quantity": 100, "customer_id": "c1"} for i in range(1, 5)]
    rows += [{"date": f"2026-01-{i:02d}", "sku": "B", "quantity": 10, "customer_id": "c2"} for i in range(1, 5)]
...
def test_backtest_selects_baseline_when_complex_model_is_worse():
    ...
    result = rolling_origin_backtest(series, [("median_baseline", median_baseline), ("bad", bad)], horizon=7)
