...
import { formatNumber, shortReason, positionWord } from '../utils/presentation';
...
export function RecommendationsTable({ products, selected, activeSku, grouped, sort, onSort, onOpen, onToggle, onToggleAll }: Props) {
  ...
  return <>
    ...
    <div className="desktop-table"><div className="table-scroll" tabIndex={0} role="region" aria-label="Таблица рекомендаций с горизонтальной прокруткой">
      <table><caption className="sr-only">Рекомендации закупок из API</caption><thead><tr>
        ...
        {grouped && (index === 0 || rows[index - 1].supplierId !== p.supplierId) && <tr className="supplier-row"><th colSpan={9} scope="rowgroup">{p.supplier} <span>Позиций: {rows.filter(r => r.supplierId === p.supplierId).length}</span></th></tr>}
