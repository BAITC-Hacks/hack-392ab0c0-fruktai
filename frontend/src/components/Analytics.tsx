...
export function Analytics({ products, onStatus }: { products: DemoProduct[]; onStatus: (status: string) => void }) {
  ...
  return <aside className="analytics-sidebar" aria-label="Аналитика текущей выборки">
    ...
    {expanded && <div id="analytics-content">
      ...
      <section className="analytics-section"><h3>План пополнения</h3><div className="plan-total"><Package size={18}/><strong>{quantitySummary(products)}</strong></div><p className="helper">{suppliers} {supplierWord(suppliers)} в выборке. Объёмы в разных единицах не складываются.</p><div className="mini-metric"><span>Товар отсутствует</span><b>{products.filter(p => p.onHand === 0).length} {positionWord(products.filter(p => p.onHand === 0).length)}</b></div><div className="mini-metric"><span>Есть товар в пути</span><b>{products.filter(p => p.inTransit > 0).length} {positionWord(products.filter(p => p.inTransit > 0).length)}</b></div></section>
