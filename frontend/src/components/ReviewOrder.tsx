...
export function ReviewOrder({ products, onExport }: Props) {
  ...
  return <>
    ...
    <div className="review-summary"><Package size={22}/><strong>{quantitySummary(products)}</strong><span>{products.length} позиций · {suppliers.length} поставщиков</span></div>
    <div className="review-groups">{suppliers.map(supplier => <section key={supplier}><h3>{supplier}</h3>{products.filter(p => p.supplier === supplier).map(p => <div className="review-row" key={p.sku}><span><strong>{p.name}</strong><small>{p.sku}</small></span><b>{formatNumber(p.recommended)} {p.unit}</b></div>)}</section>)}</div>
