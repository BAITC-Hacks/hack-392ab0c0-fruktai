...
export function DataView({ data }: { data: DemoSnapshot }) {
  return <section className="data-view"><div className="view-heading"><h2>Данные расчёта</h2>
    <p>Результат FastAPI по набору {data.dataset}, сохранённый в SQLite.</p></div>
    ...
    <p className="data-notice">{data.dataset === 'demo' ? 'Набор учебный.' : 'Использованы загруженные источники.'} Количество рассчитано по нормализованным данным. Прямое соединение с 1С не настроено.</p>
    {data.metadata && <><p>Склад: {data.metadata.warehouse_id || 'не указан'}</p><ul>{data.metadata.warnings.map(w => <li key={w}>{w}</li>)}</ul></>}
    ...
    </dl><p>Категория и единица отображаются из загруженного справочника. Пропуски обозначаются «Не указана» и «ед.».</p>
  </section>;
}
