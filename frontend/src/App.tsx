...
export default function App() {
  ...
  return <>
    ...
    <main id="main-content">
      ...
      <div className="context-row"><span>Демонстрационные данные</span><span><CalendarDays size={14}/>{data ? 'Срез: ' + formatDate(data.updatedAt) + ' · ' + new Date(data.updatedAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Almaty' }) + ' (UTC+5)' : 'Дата среза не получена'}</span>{data && <button className="context-link" onClick={() => setView('data')}>Источник и полнота данных <ChevronRight size={12}/></button>}</div>
      ...
      {loading && !data ? <LoadingState/> : data && <>
        ...
        <div className={'workspace ' + (!showAnalytics || view === 'data' ? 'without-analytics' : '')} id="workspace">
          <div className="workspace-content">
            ...
            {view === 'data' ? <><DataView data={data}/><AgentRunPanel open={agentOpen} onChange={setAgentOpen}/></> : filtered.length === 0 ? <div className="empty-state"><Search size={30}/><h2>Подходящих позиций нет</h2><p>Попробуйте другой запрос или сбросьте фильтры.</p><button className="secondary-button" onClick={() => setFilters(initialFilters)}>Сбросить фильтры</button></div> : view === 'comparison' ? <Comparison products={filtered}/> : <RecommendationsTable products={filtered} selected={selected} activeSku={activeProduct?.sku ?? null} grouped={grouped} sort={filters.sort} onSort={sort => setFilters({ ...filters, sort })} onOpen={setActiveProduct} onToggle={toggle} onToggleAll={toggleAll}/>}
