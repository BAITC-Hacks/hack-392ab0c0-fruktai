...
export default function App() {
  ...
  return <>
    ...
    <header className="app-header"><div className="brand"><span className="brand-mark" aria-hidden="true"><i/><i/><i/><i/></span><div><strong>Электрокомплект</strong><span>ekt.kz <span className="brand-divider">/</span> Закупки</span></div></div>
      {data && <nav className="header-nav" aria-label="Основная навигация"><button className={view !== 'data' && !grouped ? 'is-active' : ''} onClick={() => { setView('table'); setGrouped(false); }}><ShoppingCart size={18}/>План закупок</button><button className={grouped ? 'is-active' : ''} onClick={() => { setView('table'); setGrouped(true); }}><Users size={18}/>Поставщики</button><button onClick={() => { setAgentOpen(true); document.getElementById('agent-run')?.scrollIntoView({ block: 'center' }); }}><ShieldCheck size={18}/>Ход анализа</button></nav>}
    ...
    <main id="main-content">
      ...
      <div className="context-row"><span><Warehouse size={14}/>Склад не указан в наборе</span><span><CalendarDays size={14}/>{data ? 'Срез: ' + formatDate(data.updatedAt) + ' · ' + new Date(data.updatedAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Almaty' }) + ' (UTC+5)' : 'Дата среза не получена'}</span>{data && <button className="context-link" onClick={() => setView('data')}>Источник и полнота данных <ChevronRight size={12}/></button>}</div>
      ...
      {loading && !data ? <LoadingState/> : data && <>
        <div className="attention-banner"><span className="alert-symbol"><AlertTriangle size={19}/></span><div><strong>Проверьте позиции с риском дефицита</strong><p>Позиций с риском в демонстрационном наборе: {riskCount}. Начните с товаров без остатка.</p></div><button onClick={() => chooseKpi('risk')}>Показать позиции <ChevronRight size={16}/></button></div>
