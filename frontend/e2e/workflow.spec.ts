...
test('dashboard -> item history -> override -> persisted API -> CSV', async ({ page, request }) => {
  ...
  const responsePromise = page.waitForResponse(
    (r) => r.url().endsWith('/api/v1/recalculate') && r.status() === 200,
  );
  await page.goto('/');
  const baseline = await (await responsePromise).json();
  const item = baseline.recommendations.find(
    (r: { recommended_qty: number }) => r.recommended_qty > 0,
  );
  ...
  await page
    .getByRole('textbox', { name: 'На складе', exact: true })
    .fill(String(item.on_hand + item.recommended_qty + 1));
  const changedPromise = page.waitForResponse(
    (r) => r.url().endsWith('/api/v1/recalculate') && r.status() === 200,
  );
  await page.getByRole('button', { name: 'Пересчитать через API', exact: true }).click();
  const changed = await (await changedPromise).json();
  expect(
    changed.recommendations.find((r: { sku: string }) => r.sku === item.sku).recommended_qty,
  ).toBe(0);
...
test('upload Excel -> preview -> calculate -> persisted history -> Excel export', async ({
  page,
  request,
}) => {
  const errors: string[] = [];
  ...
  await page
    .getByRole('checkbox', { name: 'Клиентские данные обезличены, персональных данных нет' })
    .check();
  ...
  await expect(
    page.getByRole('heading', { name: 'Вход проверен — проверьте полноту перед расчётом' }),
  ).toBeVisible();
  ...
  const history = page.waitForResponse(
    (r) => r.url().includes('/api/v1/items/') && r.url().includes(result.run_id),
  );
  await page.getByRole('button', { name: 'Открыть расчёт: ' + item.name, exact: true }).click();
  ...
  await page.reload();
  await expect(
    page.getByRole('button', { name: 'Вернуться к учебному набору demo' }),
  ).toBeVisible();
  expect(errors).toEqual([]);
});
...
test('new workspace: navigation, sort, filters, focus and accessible drawers', async ({ page }) => {
  ...
  await page.screenshot({
    path: '../artifacts/design-desktop.png',
    fullPage: true,
    animations: 'disabled',
  });
  ...
  await expect(page.getByTestId('sales-history')).toBeVisible();
  await page.screenshot({
    path: '../artifacts/design-details.png',
    fullPage: true,
    animations: 'disabled',
  });
  await page.keyboard.press('Escape');
  ...
  await page.screenshot({
    path: '../artifacts/design-overview.png',
    fullPage: true,
    animations: 'disabled',
  });
  ...
  await page.screenshot({
    path: '../artifacts/design-sources.png',
    fullPage: true,
    animations: 'disabled',
  });
});
...
test('mobile workspace fits viewport and preserves working item actions', async ({ page }) => {
  ...
  await page.screenshot({
    path: '../artifacts/design-mobile.png',
    fullPage: true,
    animations: 'disabled',
  });
  await page
    .locator('.mobile-product')
    .first()
    .getByRole('button', { name: /Открыть расчёт/ })
    .click();
  await expect(page.getByRole('dialog')).toBeVisible();
  ...
  await page
    ...
    .getByRole('button', { name: 'Обзор запасов', exact: true })
    .click();
...
test('API error is actionable and does not claim a successful calculation', async ({ page }) => {
  let fail = true;
  await page.route('**/api/v1/recalculate', (route) =>
    fail
      ? route.fulfill({
          status: 503,
          contentType: 'application/json',
          body: JSON.stringify({ detail: 'Тест: сервер временно недоступен' }),
        })
      : route.continue(),
  );
  await page.goto('/');
